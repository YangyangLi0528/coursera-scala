...
object Huffman {
   ...
   */
    ...
    def times(chars: List[Char]): List[(Char, Int)] =
      (chars.sortWith(_<_)).group().map(xs => (xs.head,xs.length))
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] ={
      ...
      //freqs.sortWith((_,_) match {case ((c,fc),(d,fd)) => fc < fd}).map(_match {case(c,w) => new Leaf(c,w)})
...
//      case l :: r :: trees_ => (new Fork(l,r,chars(l) ::: chars(r),weight(l) + weight(r)) :: trees_).sortWith(weight((_) < weight(_))
//      case _ => trees
//    }
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case l :: r :: trees_ => (new Fork(l, r, chars(l) ::: chars(r), weight(l) + weight(r)) :: trees_).sortWith(weight(_) < weight(_))
